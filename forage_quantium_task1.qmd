...
productWords[, words_clean := trimws(
  gsub(" +", " ",
       gsub("[Gg]$", "", # remove trailing 'g' only
            gsub("[^A-Za-z ]", "", words) # remove non-letter/space chars
       )
  )
)]
...
word_list <- strsplit(productWords$words_clean, " ")

# Flatten the list into one vector of words
all_words <- unlist(word_list)

# Count and sort by frequency
word_freq <- data.table(word = all_words)[, .N, by = word][order(-N)]
head(word_freq)
...
sapply(transactionData, function(x) sum(is.null(x)))
...
outlier_customer <- unique(outlier$LYLTY_CARD_NBR) # 226000
```
...
transaction_counts <- transactionData[, .N, by = DATE][order(DATE)]
...
# Replace NA with 0, order by DATE
...
transactions_by_day_DEC <- transactions_by_day[month(DATE) == 12]
...
customerData[, .N, LIFESTAGE][order(LIFESTAGE)]
customerData[, .N, PREMIUM_CUSTOMER][order(PREMIUM_CUSTOMER)]
```
...
sales_summary <- data %>%
  ...
  summarise(TOT_SALES_GROUPED = sum(TOT_SALES, na.rm = TRUE))
sales_summary

plot_sales_life_customer <- ggplot(sales_summary, aes(x = LIFESTAGE, y = TOT_SALES_GROUPED, fill = PREMIUM_CUSTOMER)) +
...
count_summary <- data %>%
  group_by(LIFESTAGE, PREMIUM_CUSTOMER) %>%
  summarise(NUM_IDS = n_distinct(LYLTY_CARD_NBR))
count_summary

plot_num_life_customer <- ggplot(count_summary, aes(x = LIFESTAGE, y = NUM_IDS, fill = PREMIUM_CUSTOMER)) +
  geom_col(position = "dodge") +
  labs(title = "Total Number of Customers by Lifestage and Premium Customer",
       x = "Lifestage",
       y = "Distinct Customer Count",
       fill = "Premium Customer") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
priceperunit_summary <- data %>%
  group_by(LIFESTAGE, PREMIUM_CUSTOMER) %>%
  summarise(PROD_QTY_GROUPED = sum(PROD_QTY, na.rm = TRUE))
priceperunit_summary
...
avg_summary <- merge(
  ...
  merge(priceperunit_summary, by = c("LIFESTAGE", "PREMIUM_CUSTOMER"), all.x = TRUE)

avg_summary$AVG_UNIT_PER_CUST = (avg_summary$PROD_QTY_GROUPED / avg_summary$NUM_IDS)
avg_summary

plot_avg_life_customer <- ggplot(avg_summary, aes(x = LIFESTAGE, y = AVG_UNIT_PER_CUST, fill = PREMIUM_CUSTOMER)) +
  geom_col(position = "dodge") +
  labs(title = "Average Number of Units per Customer by Lifestage and Premium Customer",
       x = "Lifestage",
       y = "Average Number of Units per Customer",
       fill = "Premium Customer") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
avg_summary$AVG_PRICE_PER_UNIT = (avg_summary$TOT_SALES_GROUPED / avg_summary$PROD_QTY_GROUPED)
avg_summary

plot_avg_life_customer <- ggplot(avg_summary, aes(x = LIFESTAGE, y = AVG_PRICE_PER_UNIT, fill = PREMIUM_CUSTOMER)) +
  geom_col(position = "dodge") +
...
filtered_data <- data %>%
  filter(LIFESTAGE == c("MIDAGE SINGLES/COUPLES", "YOUNG SINGLES/COUPLES")) %>%
  mutate(GROUP = ifelse(PREMIUM_CUSTOMER == "Mainstream", "Mainstream", "Non_Mainstream"))
head(filtered_data)

ttest <- t.test(TOT_SALES ~ GROUP, data = filtered_data)
ttest
```

The t-test results in a p-value of `r ttest$p.value`, i.e. the unit price for mainstream, young and mid-age singles and couples \[`ARE` / ARE NOT\] significantly higher than that of budget or premium, young and midage singles and couples.
...
# Filtered dataset
filtered_main_ysc <- data %>%
  filter(LIFESTAGE == "YOUNG SINGLES/COUPLES", PREMIUM_CUSTOMER == "Mainstream")
head(filtered_main_ysc)

# Transform into transactions
# https://www.geeksforgeeks.org/apriori-algorithm-in-r-programming/
trans_data_brand <- as(split(filtered_main_ysc$BRAND, filtered_main_ysc$TXN_ID), "transactions")

# Obtain and look into association rules
apriori_rules_brand <- apriori(trans_data_brand, parameter = list(supp = 0.1, conf = 0.1)) 
inspect(apriori_rules_brand[1:min(10, length(apriori_rules_brand))]) 

# Bar plot to inspect the distribution of objects based on the transactions for item frequencies/support
max_freq_brand <- max(itemFrequency(trans_data_brand, type = "relative"))
arules::itemFrequencyPlot(trans_data_brand, topN = 20, 
                          col = brewer.pal(8, 'Pastel2'),
                          main = 'Relative Brand Frequency Plot',
                          type = "relative",
                          xlab = "Brand",
                          ylab = "Frequency (Relative)",
                          ylim = c(0, max_freq_brand + 0.05))
```

We can see that : Kettle provides the greatest item frequency of `r itemFrequency(trans_data_brand, type = "relative")["Kettle"]`, which is significantly greater than the corresponding item frequencies for Doritos and Pringles at `r itemFrequency(trans_data_brand, type = "relative")["Doritos"]` and `r itemFrequency(trans_data_brand, type = "relative")["Pringles"]` respectively. As such, it can be inferred that customers under the segment `Mainstream - young singles/couples` particularly prefer `"Kettle"` brand chips.
...
ggplot(filtered_main_ysc, aes(x = PACK_SIZE)) +
  geom_density(fill = "skyblue", alpha = 0.6) +
  labs(title = "Density Plot of Pack Size",
       x = "Pack Size (g)",
       y = "Density") +
  theme_minimal()

ggplot(data, aes(x = PACK_SIZE)) +
  geom_density(fill = "skyblue", alpha = 0.6) +
  labs(title = "Density Plot of Pack Size",
       x = "Pack Size (g)",
       y = "Density") +
...
# Transform into transactions
# https://www.geeksforgeeks.org/apriori-algorithm-in-r-programming/
trans_data_pack <- as(split(filtered_main_ysc$BRAND, filtered_main_ysc$TXN_ID), "transactions")

# Get item frequencies
freqs <- itemFrequency(trans_data_pack, type = "relative")

# Convert item names (pack sizes) to numeric
pack_sizes <- as.numeric(names(freqs))

# Order by numeric pack size
ordered_indices <- order(pack_sizes)
ordered_freqs <- freqs[ordered_indices]

# Plot as barplot
barplot(ordered_freqs[1:20], 
        names.arg = names(ordered_freqs)[1:20],
        col = brewer.pal(8, 'Pastel2'),
        main = "Relative Pack Size Frequency Plot (Ordered by Pack Size)",
        xlab = "Pack Size",
        ylab = "Frequency (Relative)",
        las = 2,  # rotate x labels
        ylim = c(0, max(ordered_freqs[1:20]) + 0.05))
```
...
segment_summary <- data %>%
  ...
  group_by(LIFESTAGE) %>%
  mutate(prop_within_lifestage = count / sum(count)) %>%
  group_by(PREMIUM_CUSTOMER) %>%
  mutate(prop_within_premium = count / sum(count)) %>%
  ungroup()
segment_summary

ggplot(segment_summary, aes(x = PREMIUM_CUSTOMER, y = LIFESTAGE, fill = count)) +
  geom_tile(color = "white") +
  scale_fill_viridis(name = "Count", option = "D", begin = 0, end = 1, direction = 1) +
  labs(title = "Count of Customers by Segment",
       x = "Premium Customer Status", y = "Lifestage", fill = "Count") +
  theme_minimal()

ggplot(segment_summary, aes(x = PREMIUM_CUSTOMER, y = LIFESTAGE, fill = prop_within_lifestage)) +
  geom_tile(color = "white") +
  scale_fill_viridis(name = "Proportion", option = "C") +
  labs(title = "Proportion within Lifestage",
       x = "Premium Customer Status", y = "Lifestage", fill = "Proportion") +
  theme_minimal()

ggplot(segment_summary, aes(x = PREMIUM_CUSTOMER, y = LIFESTAGE, fill = prop_within_premium)) +
  geom_tile(color = "white") +
  scale_fill_viridis(name = "Proportion", option = "C") +
  labs(title = "Proportion within Premium Customer Type",
       x = "Premium Customer Status", y = "Lifestage", fill = "Proportion") +
  theme_minimal()
```