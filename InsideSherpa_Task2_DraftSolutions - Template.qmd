...
measureOverTime <- data[, .(totSales = ___, 
                            nCustomers = ___, 
                            nTxnPerCust = ___,
                            nChipsPerTxn = ___,
                            avgPricePerUnit = ___)
...
calculateCorrelation <- function(inputTable, metricCol, storeComparison) {
  ...
  for (i in storeNumbers) {
    calculatedMeasure = data.table("Store1" = ___, 
                                   "Store2" = ___,
                                   "corr_measure" = ___)
...
trial_store <- ___
corr_nSales <- calculateCorrelation(___, quote(), ___)
corr_nCustomers <- calculateCorrelation(___, quote(), ___)
...
corr_weight <- 0.5
score_nSales <- merge(___, ___, by = ___)[, scoreNSales := ___]
score_nCustomers <- merge(___, ___, by = ___)[, scoreNCust := ___]
```
...
score_Control <- merge(___, ___, by = ___)
...
percentageDiff <- merge(___, 
                        ___,
                        by = ___)[, percentageDiff := ___]
```
...
percentageDiff[, tValue := ___][, TransactionMonth := ___][, .()]
```
...
pastSales <- measureOverTimeSales[, Store_type := ___
                                  ][, totSales := ___
                                    ][, TransactionMonth := ___
...
scalingFactorForControlCust <- ___

measureOverTimeCusts <- measureOverTime

scaledControlCustomers <- measureOverTimeCusts[, 
                                               ][, controlCustomers := ___
                                                   ][, Store_type := ___
                                                       ]
percentageDiff <- ___
```
...
ggplot(___) + 
  geom_rect(data = ___, aes(xmin = ___, xmax = ___, ymin = ___, ymax = ___, color = ___), show.legend = FALSE) + 
  geom_line() + 
  labs()
```
...
measureOverTimeCusts <- measureOverTime
pastCustomers <- measureOverTimeCusts[, Store_type := ___
                                      ][, numberCustomers := ___, by = ___
                                        ][, TransactionMonth := ___
                                            ][YEARMONTH < 201903 , ]
ggplot(___) + 
  geom_line() + 
...
percentageDiff <- merge(___, ___, by = "YEARMONTH")[, percentageDiff := ___]
...
pastSales_Controls95 <- pastSales[Store_type == ___,
                                  ][, totSales := ___
                                  ][, Store_type := "Control 95th % confidence interval"]

pastSales_Controls5 <- pastSales[Store_type == ___, 
                                 ][, totSales := ___
                                 ][, Store_type := "Control 5th % confidence interval"]
...
scalingFactorForControlSales <- ___

#### Apply the scaling factor
measureOverTimeSales <- measureOverTime
scaledControlSales <- ___
...
#### Control store 95th percentile
pastSales_Controls95 <- ___

#### Control store 5th percentile
pastSales_Controls5 <- ___
...
#### Control store 95th percentile
pastCustomers_Controls95 <- ___

#### Control store 5th percentile
pastCustomers_Controls5 <- ___
